...
function copyNode(node){
	let newNode = new entidades.Nodo(node.tamanyo, node.ejeX, node.ejeY);
...
function revive(k,v){
	if (v instanceof Object && v._class == 'Matriz')
		return entidades.Matriz.from(v);
	if (v instanceof Object && v._class == 'Nodo')
		return entidades.Nodo.from(v);
	return v;
}

//Define el tipo de objeto que se guarda.
function replacer(k,v){
	if (v instanceof entidades.Matriz)
		return entidades.Matriz.to(v);
	if (v instanceof entidades.Nodo)
		return entidades.Nodo.to(v);
	return v;
}
